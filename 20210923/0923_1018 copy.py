...
def get_least_by_idx(nIdxCol: int, tplSize: tuple, lstBoard: list) -> int:
    ...
    for idx_row in range(n_retry):
        ...
        return min(n_count, 64-n_count) # 시작 타일이 W인경우와 B인경우에서 칠할 칸수의 합은 64


def solution(tplSize: tuple, lstBoard: list) -> int:
    n_processes: int = tplSize[1] - DEFAULT_SIZE + 1

    lst_result: list = [ get_least_by_idx(i, tplSize, lstBoard) for i in range(n_processes) ]
    print(lst_result)
