...
# 실행시간에 대한 우려로 multiprocessing 라이브러리를 활용하려했으나 런타임 에러가 나서 직렬로 처리하게 됨
DEFAULT_SIZE = 8
lst_result = []

def get_least(nIdxCol: int, tplSize: tuple, lstBoard: list) -> int:
    ...
    for idx_row in range(n_retry):
        ...
        lst_result.append(min(n_count, 64-n_count)) # 시작 타일이 W인경우와 B인경우에서 칠할 칸수의 합은 64
...
def solution(tplSize: tuple, lstBoard: list) -> int:
    ...
    for i in range(n_processes):
        get_least(i, tplSize, lstBoard) 


    return min(lst_result)
